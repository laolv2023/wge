...
* **输出目标**：命中规则后，将告警写入独立的 `wge.alerts` Topic，由独立 Adapter 完成协议转换与上下文注入。
...
| **WGE 告警属性 (`wge.alerts`)** | **Akto Protobuf 字段 (`MaliciousEventMessage`)** | **映射策略与源码级依据**                                                                                                            |
| --------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
...
Adapter 作为独立 Sidecar 服务，消费 `wge.alerts`，严格执行以下五大防护机制，确保 Akto Backend 的稳定与 GUI 的完美渲染：
...
* WGE 侧引入 Akto Protobuf SDK，订阅 `akto.api.logs2`，输出标准化 JSON 至 `wge.alerts`。
